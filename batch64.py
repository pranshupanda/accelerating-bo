...
CONFIG = {
    ...
    "print_every": 10,
    "optimizer_defaults": {
        "BCA": {"r": 0.9, "beta": 3.095, "max_iters": 200, "batch_size": 32},
...
class BCA(OptBase):
    def __init__(self, dim, acq_func, max_iters, r, beta, batch_size):
    ...
    def fast_chm(self, X):
        ...
        Batched Fast-CHM.
        Args: 
            X: Binary Vector X [0,1]
        Output:
            Y : Batch of mutated tensors 
            best_vals: acquisition function values of tensors in Y 
        """
        B, n = X.shape
        X_mut = X.clone()

        best_vals = safe_acq_eval(self.acq_func, X)   
        Y = X.clone()
        ...
        for i in range(n):
            ...
            flip_mask = (torch.rand(B, device=device) < self.r).float()

            X_mut[torch.arange(B), idx] = (
                X_mut[torch.arange(B), idx] * (1 - flip_mask) +
                (1 - X_mut[torch.arange(B), idx]) * flip_mask
            )

            pi = max(min(i + 1, n - i) ** (-self.beta), 1.0 / n)
            accept = (torch.rand(B, device=device) < pi)
            curr_vals = safe_acq_eval(self.acq_func, X_mut)
            better = (curr_vals >= best_vals) & accept
            Y[better] = X_mut[better]
            best_vals[better] = curr_vals[better]
    ...
    def optimize(self):
        ...
        X = (torch.rand(self.batch_size, self.dim, device=device) < 0.5).float()
        vals = safe_acq_eval(self.acq_func, X)

        for _ in range(self.max_iters):
            Y, new_vals = self.fast_chm(X)
